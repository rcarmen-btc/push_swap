...
/*   Updated: 2021/06/16 23:38:48 by rcarmen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	first_phase_a_small_half_to_b(t_head *head)
{
	...
	// while (head->a->order < head->next - 1)	
	// {
	// 	ra(&head);
	// 	// head->next++;
	// }
...
void	third_phase_a_ones_to_b_and_back(t_head *head)
{
	...
	while (head->a->flag != 0)
	{
		...
		{
			while (!checking_for_sorting(head) && head->a->order == head->next)	
			{
				printf("heeeee====\n");
				ra(&head);
				head->next++;
...
void	fourth_pre_cycle_ending(t_head *head)
{
	int save;
